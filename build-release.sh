...
build_target() {
	...
	local client_out="${BUILD_DIR}/kcpc_${os}_${suffix}${ext}"
	local server_out="${BUILD_DIR}/kcps_${os}_${suffix}${ext}"
	...
	# Execute Go build
	go build -mod=vendor -ldflags "${LDFLAGS}" -o "${client_out}" "${CLIENT_SRC}" || {
		echo "Error: Client compilation failed for ${os}/${suffix}"
		return 1
	}
	go build -mod=vendor -ldflags "${LDFLAGS}" -o "${server_out}" "${SERVER_SRC}" || {
		echo "Error: Server compilation failed for ${os}/${suffix}"
		return 1
	}
...
package_target() {
	...
	local client_bin="kcpc_${os}_${suffix}${ext}"
	local server_bin="kcps_${os}_${suffix}${ext}"
